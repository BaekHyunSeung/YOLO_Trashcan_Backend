...
trashcans_detail_alias = APIRouter(prefix="/trashcans_list")
...
@trashcans_detail_alias.get("/{trashcan_id}/connection-test")
async def test_trashcan_connection_alias(trashcan_id: int, db: SessionDep):
    result = await service.test_trashcan_connection(trashcan_id, db)
    if result is None:
        raise HTTPException(status_code=404, detail="Trashcan not found")
    return result
...
@trashcans_detail_alias.get("/{trashcan_id}")
async def get_trashcan_alias(trashcan_id: int, db: SessionDep):
    result = await service.get_trashcans_detail(trashcan_id, db)
    if result is None:
        raise HTTPException(status_code=404, detail="Trashcan not found")
    return result
...
@trashcans_detail_alias.get("/{trashcan_id}/waste-detail")
async def get_trashcan_waste_detail_alias(trashcan_id: int, db: SessionDep):
    result = await service.get_waste_detail(trashcan_id, db)
    if result is None:
        raise HTTPException(status_code=404, detail="Trashcan not found")
    return result