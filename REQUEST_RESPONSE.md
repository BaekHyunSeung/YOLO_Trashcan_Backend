...
**Response 예시**
```json
{ "saved": true, "detection_id": 1, "total_objects": 2 }
```

**저장 조건**
- 탐지 결과(종류별 개수)가 이전 저장과 다르면 즉시 저장
- 결과가 같으면 `DETECTION_SAVE_MIN_INTERVAL_SECONDS`(기본 10초) 이후에만 저장 여부 판단

**Response (저장 안 함)**
```json
{ "saved": false, "reason": "cooldown", "next_allowed_in": 12 }
```
```json
...
**Query**
- `waste_type` (기본 전체): `전체|플라스틱|유리병|캔|스티로폼` 또는 영문(`Plastic|GlassBottle|Can|Styrofoam|PETBottle`)
- `offset` (기본 0)
- `limit` (기본 50, 최대 200)
...
```json
{
  ...
  "server_url": "http://127.0.0.1:9000/health",
...
**설명**
- `server_url`에 GET 요청하여 200이면 온라인
- 200이 아니면 미연결로 판단
...
## 8) 쓰레기 종류 시드 등록
**Request**: `POST /waste-types/seed?types=Plastic&types=PETBottle&types=Can&types=Styrofoam`
...
```json
{
  "total_objects": 2,
  "total_events": 1,
  "by_type": { "Plastic": 2, "PETBottle": 3, "GlassBottle": 0, "Styrofoam": 0 },
  "by_type_list": [
    { "name": "Plastic", "count": 2 },
    { "name": "PETBottle", "count": 3 },
    { "name": "GlassBottle", "count": 0 },
    { "name": "Styrofoam", "count": 0 }
  ],
  "by_type_items": [
    { "waste_type_id": 2, "type_name": "Plastic", "count": 2 },
    { "waste_type_id": 1, "type_name": "PETBottle", "count": 3 },
    { "waste_type_id": 4, "type_name": "GlassBottle", "count": 0 },
    { "waste_type_id": 5, "type_name": "Styrofoam", "count": 0 }
  ]
}
```
  ...
  "items": [
    {
      ...
      "total_events": 1,
      "total_objects": 2,
      "by_type": { "Plastic": 1, "PETBottle": 1 },
      "by_type_list": [
        { "name": "Plastic", "count": 1 },
        { "name": "PETBottle", "count": 1 }
      ],
      "by_type_items": [
        { "waste_type_id": 2, "type_name": "Plastic", "count": 1 },
        { "waste_type_id": 1, "type_name": "PETBottle", "count": 1 }
      ]
    }
  ]
}
...
```json
{
  ...
  "total_events": 1,
  "total_objects": 2,
  "by_type": { "Plastic": 1, "PETBottle": 1, "GlassBottle": 0, "Styrofoam": 0 },
  "by_city": { "Seoul": 2, "Busan": 0 },
  "by_type_list": [
    { "name": "Plastic", "count": 1 },
    { "name": "PETBottle", "count": 1 },
    { "name": "GlassBottle", "count": 0 },
...
- `stale_hours`는 하위 호환용 파라미터(현재는 서버 응답 기반 판정)
